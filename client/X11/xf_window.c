...
void xf_SetWindowDecorations(xfContext* xfc, xfWindow* window, BOOL show)
{
	...
	retval = XChangeProperty(xfc->display, window->handle, xfc->_MOTIF_WM_HINTS, xfc->_MOTIF_WM_HINTS, 32,
...
void xf_ResizeDesktopWindow(xfContext* xfc, xfWindow* window, int width, int height)
{
	...
	if (size_hints)
	{
		...
		size_hints->min_width = xfc->width;
		size_hints->max_width = xfc->width;
		size_hints->min_height = xfc->height;
		size_hints->max_height = xfc->height;
...
{
	DEBUG_X11_LMS("local_move");


	if (window->local_move.state != LMS_NOT_ACTIVE)
		return;

	DEBUG_X11_LMS("sssss	direction=%d window=0x%X rc={l=%d t=%d r=%d b=%d} w=%d h=%d   "
